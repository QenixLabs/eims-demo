        ...
        <CardContent className="space-y-4">
          {!paymentExists ? (
            <>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="government_voucher">{t("governmentVoucher")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              >
                {isCreatingPayment ? (
                  <><Loader2 size={16} className="mr-2 animate-spin" />{t("creating")}</>
                ) : (
                  <><Receipt size={16} className="mr-2" />{t("recordPayment")}</>
                )}
              </Button>
            </>
          ) : (
            <div className="space-y-4">
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center justify-between mb-3">
                  <h5 className="text-sm font-semibold text-slate-900">{t("recorded")}</h5>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <p className="text-slate-500 text-xs">{t("amountLabel")}</p>
                    <p className="font-bold text-slate-900">${payment.amount}</p>
                  </div>
                  <div>
                    <p className="text-slate-500 text-xs">{t("methodLabel")}</p>
                    <p className="font-medium text-slate-700 capitalize">
                      {payment.paymentMethod.replace("_", " ")}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500 text-xs">{t("receiptNo")}</p>
                    <p className="font-mono text-xs text-slate-600">{payment.receiptNumber}</p>
                  </div>
                  <div>
                    <p className="text-slate-500 text-xs">{t("status")}</p>
                    <p className="font-medium text-emerald-600">{t("completed")}</p>
                  </div>
                </div>
              </div>
      ...
      <Card className="border-slate-200 shadow-sm">
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
            <h5 className="text-sm font-semibold text-blue-900 mb-3">{t("breakdown")}</h5>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-blue-700">{t("identityCardFee")}</span>
                <span className="font-medium text-blue-900">$35.00</span>
              </div>
              <div className="flex justify-between">
                <span className="text-blue-700">{t("biometricEnrollment")}</span>
                <span className="font-medium text-blue-900">$10.00</span>
              </div>
              <div className="flex justify-between">
                <span className="text-blue-700">{t("processingFee")}</span>
                <span className="font-medium text-blue-900">$5.00</span>
              </div>
              <div className="border-t border-blue-200 pt-2 flex justify-between">
                <span className="font-semibold text-blue-900">{t("total")}</span>
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
            <h5 className="text-sm font-semibold text-slate-900 mb-3">{t("instructions")}</h5>
          ...
          <div className="flex items-start gap-2 p-3 bg-emerald-50 rounded-xl border border-emerald-200">
            <CheckCircle2 size={16} className="text-emerald-600 mt-0.5 shrink-0" />
            <div>
              <p className="text-xs font-medium text-emerald-900">{t("important")}</p>
